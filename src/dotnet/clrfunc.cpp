...
Handle<v8::Function> ClrFunc::Initialize(System::Func<System::Object^,Task<System::Object^>^>^ func)
{
    ...
    if (proxyFactory.IsEmpty())
    {
		...
		proxyFunction.Reset(clrFuncProxyFunction->GetIsolate(), clrFuncProxyFunction);
		...
		proxyFactory.Reset(codeFunction->GetIsolate(), codeFunction);
    }
	...
	Nan::Persistent<Function> cb(funcProxy);
	cb.SetWeak((void*)wrap, &clrFuncProxyNearDeath, WeakCallbackType::kParameter);
